...
public class TC130_CCC_GeneralCounsel_Regular_ContinueWithoutSignIn_CreditCard_RejectEmail extends BaseClass {
	...
	static String testCaseName = ReadProperties.getConfig("NS_TC130_TestcaseName");
	...
	@Test(groups = {"tc=947632"}, description = "CoCounsel Core GeneralCounsel Regular Continue Without SignIn RejectEmail payment through CreditCard")
	public void CCC_GeneralCounsel_Regular_ContinueWithoutSignIn_CreditCard_RejectEmail() {
		...
		String CreditCard = testdataMap.get("CreditCard");
		...
		checkoutPage.verifyLandedOnCheckoutPage().enterFirstName().enterLastName().enterExistingEmailID(ActiveTrialsEmail).enterPhoneNumber()
				.enterOrganizationName().selectType(sectorFromExcel).enterAddress().clickOnContinueToPayment().clickOnContinueWithOutSignIn()
				...
				.selectCreditCard().enterCardHolderName().enterCardNumber(CreditCard).selectMonth()
				.selectYear().enterSecurityCode(3).clickOnAuthorizeCheckBox().clickOnPlaceOrder();
