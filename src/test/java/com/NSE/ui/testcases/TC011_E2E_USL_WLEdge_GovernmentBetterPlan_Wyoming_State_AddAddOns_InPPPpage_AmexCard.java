...
public class TC011_E2E_USL_WLEdge_GovernmentBetterPlan_Wyoming_State_AddAddOns_InPPPpage_AmexCard extends BaseClass {
	...
	static String testCaseName = ReadProperties.getConfig("NS_TC011_TestcaseName");
	...
	@Test(groups = {
			"tc:591102" }, description = "E2E_USL_WLEdge_GovernmentBetterPlan_Wyoming_State_AddAddOns_InPPPpage_AmexCard")
	public void E2E_USL_WLEdge_GovernmentBetterPlan_Wyoming_State_AddAddOns_InPPPpage_AmexCard() throws Exception {
		...
		String changePlanDuration = testdataMap.get("DurationPlanSummary");
		...
		pppage.verifyLandedOnWestLawEdgePage().validateWLEdgeBanner().validateDefaultSector().validateDefaultPlan()
				...
				.selectPlan(planFromExcel).changeDurationInPlanSummarySection(changePlanDuration).getPlanDetails()
				.validatePlanSummaryWithAddOnsSection(sectorFromExcel, actualAttorneysFromExcel, changePlanDuration,
		...
		checkoutPage.verifyLandedOnCheckoutPage().enterFirstName().enterLastName().enterEmail().enterPhoneNumber()
				.enterOrganizationName().selectType(sectorFromExcel).enterStateAddress("Wyoming")
				...
				.verifyPlanDurationInCart(changePlanDuration).verifyNumberOfAttorneysInCart(actualAttorneysFromExcel)
				.clickOnContinueToPayment().selectCreditCard().enterCardHolderName().enterCardNumber(CreditCard)
				.selectMonth().selectYear().enterSecurityCode(4)
