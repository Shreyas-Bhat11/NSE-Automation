...
public class TC084_E2E_USL_PL_GeneralCounsel_BetterPlan_MultinationalPlan_With_YesBankAccount extends BaseClass {
	...
	static String testCaseName = ReadProperties.getConfig("NS_TC084_TestcaseName");
	...
	@Test(groups = {
			"tc=831128" }, description = "E2E flow for USL PL sector GeneralCounsel with plan BetterPlan select MultinationalPlan Payment YesBankAccount")
	public void E2E_USL_PL_GeneralCounsel_BetterPlan_MultinationalPlan_With_YesBankAccount() {
		...
		pppage.verifyLandedOnPracticalLawPage().validatePLBanner().validateDefaultSector().validateDefaultPlan()
				...
				.selectSector(sectorFromExcel).validateSector(sectorFromExcel).selectMultinatonalPlan()
...
//				.validatePlanNameInCartItem(testdataMap.get("Jurisdiction"), sectorFromExcel)
				.verifyPlanDurationInCart(planDurationFromExcel).verifyNumberOfSeatsInCart(actualAttorneysFromExcel)
